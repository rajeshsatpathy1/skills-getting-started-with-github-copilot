  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants HTML (hide bullets; show delete icon next to each participant)
        ...
        if (details.participants.length === 0) {
          ...
          participantsHtml = `<ul class="participants-list">` +
            details.participants.map(p =>
              ...
              `<button class="delete-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}" title="Remove participant">✖</button></li>`
            ).join("") +
            `</ul>`;
        }
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        messageDiv.textContent = result.message;
        messageDiv.className = "success";
        // Refresh activities so participants and availability reflect the removal
        fetchActivities();
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
        messageDiv.className = "error";
